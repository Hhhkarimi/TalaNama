...
// مقادیر روزانه میان نقاط مرجع و OHLC تاریخی بازسازی‌شده‌اند و فید رسمی نیستند.
...
export function generateMarketData() {
  ...
  for (let time = start, i = 0; time <= end; time += DAY, i += 1) {
    ...
    rows.push({
      ...
      usdOpen: usd * (1 - noise(i + 9, 0.0025)),
      usdHigh: usd * (1 + dailyWave * 0.42),
      usdLow: usd * (1 - dailyWave * 0.36)
    });
  }
...
export function applyMarketSnapshot(data, snapshot) {
  ...
  const usdOpen = existingIndex >= 0 ? existing.usdOpen : previous.usd;
  ...
  const liveRow = {
    ...
    usd: values.usd,
    premium,
    currentSourceStatus: snapshot.status || "unknown",
    globalOpen,
    ...
    usdOpen,
    usdHigh: Math.max(usdOpen, values.usd, existingIndex >= 0 ? existing.usdHigh : values.usd),
    usdLow: Math.min(usdOpen, values.usd, existingIndex >= 0 ? existing.usdLow : values.usd)
  };
